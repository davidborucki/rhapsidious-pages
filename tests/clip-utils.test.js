...
test("grid markup uses lazy images and never embeds video players", function () {
  ...
  const rendererEnd = appSource.indexOf("function renderSavedClip", rendererStart);
...
test("grid CSS restricts layouts to three and four portrait columns", function () {
  ...
  assert.match(css, /@container clip-grid \(min-width: 1160px\)\s*{\s*\.clip-grid\s*{[^}]*repeat\(4,/s);
